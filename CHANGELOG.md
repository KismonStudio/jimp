...
All notable changes to AUREON are documented in this file.
...
- Generated source-complexity sandbox limits, bilingual normative specifications, cross-module tests, and the runnable `examples/p8-types.aur` sample.
...
- Portable `.abc` 2.9 generic `STRING_LENGTH`, `STRING_LOAD`, `STRING_SLICE`, and `STRING_CONCAT` instructions with independent JavaScript/Rust verification and deterministic bounds failures.
...
- Portable `.abc` 2.8 generic `HEAP_REPLACE` and resource-bounded `HEAP_EQUAL` instructions, independently verified and executed by JavaScript and Rust without exposing reference identity.
...
- Portable `.abc` 2.7 generic heap foundation with opaque `HEAP_REF`, atomic immutable allocation, typed indexed access, length access, independent JavaScript/Rust verification, and inspector support.
...
- `.abc` bytecode inspector with human-readable disassembly and JSON output.
- Inspector validation for instruction bounds, UTF-8 operands, termination, and trailing data.
- Formal AUREON v1 source-syntax specifications in English and Portuguese.
...
- Portable `.abc` 2.0 module encoder and decoder with a section directory, scalar constant pool, typed host-import table, function metadata, and ISA-driven code encoding.
...
- Cross-language JavaScript-to-Rust validation for portable `.abc` 2.0 modules.
...
- Machine-readable `aureon-reference-sandbox` v1 limits with generated JavaScript, Rust, English, and Portuguese artifacts.
...
- Machine-readable `aureon-error-v1` definitions with generated JavaScript, Rust, English, and Portuguese artifacts.
...
- Optional `.abc` debug section mapping global instruction offsets to one-based source lines.
...
- End-to-end coverage carrying source locations from `.aur` compilation into Rust JSON errors.
...
- Initial `std:console` and `std:math/i64` API contracts separated into portable AUREON functions and declarative Host ABI bridges.
...
- Canonical portable AUREON implementations for `std:math/i64`, validated for syntax, semantics, exact catalog signatures, and absence of host imports during documentation generation.
...
- Module-qualified compiler diagnostics and optional `moduleId` metadata in `aureon-error-v1` source locations.
- Secure project-root resolver with strict relative `.aur` specifiers, UTF-8 source snapshots, real-path containment, physical and case-alias detection, source-order graph traversal, mutation checks, and cycle rejection.
...
- Portable `.abc` 2.6 debug metadata carrying portable module IDs with source lines, independently validated and reported by the inspector and Rust runtime.
...
- Optional `.abc` 2.6 build metadata for the target profile, standard-library major, entry module, and sorted target guarantees.
...
- Unified installable `aureon` CLI with `run`, `compile`, `check`, `inspect`, `init`, help, and version commands.
...
- Public `aureon-conformance-v1` suite covering language, bytecode, Host ABI, standard-library, target-profile, diagnostics, sandbox, and compatibility contracts without compiler-internal imports.
...
- Source-buffer `aureon repl` sessions with explicit edit/run commands and fresh-VM execution through the normal compiler and runtime trust boundary.
...
- Generic declarations use one verified uniform-boxed function representation across modules instead of monomorphized copies; variants and matching lower to existing `.abc` 2.9 heap and control-flow instructions.
- The active compiler and runtime now use exact portable `.abc` format `2.9`; legacy format 1 and portable formats `2.0` through `2.8` are no longer accepted.
...
- Binary AUREON bytecode v1 with `AURN` magic number, versioning, `PRINT`, and `HALT` instructions.
...
- End-to-end example at `examples/hello.aur`.
