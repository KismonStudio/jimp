...
JIMP publishes compatibility as separate contracts so an implementation can support or reject each layer explicitly.
...
```powershell
node tools/run-conformance.js --jimp=C:\path\to\jimp.js --runtime=C:\path\to\jimp-runtime.exe
```
